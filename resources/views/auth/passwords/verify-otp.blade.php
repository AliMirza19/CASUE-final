...
@section('title', 'Verify OTP - CAUSE Smart Society')
...
<div class="h-screen w-full flex items-center justify-center main-bg p-6 lg:p-0">
    <div class="w-full lg:w-[480px] glass-card p-10 lg:p-14 animate-in">
        <div class="mb-10 pt-2 relative">
            <h2 class="text-4xl font-bold text-white mb-2">Enter OTP</h2>
            <p class="text-gray-500 font-medium">We sent a 6-digit code to your email</p>
        </div>
        ...
        <form method="POST" action="{{ route('password.otp.submit') }}" class="space-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[11px] font-black text-purple-500 uppercase tracking-widest ml-1 text-center block w-full">6-Digit Code</label>
                <div class="input-box flex items-center rounded-2xl px-5 group">
                    <input 
                        ...
                        maxlength="6"
                        autocomplete="off"
                        class="w-full bg-transparent border-none focus:ring-0 py-6 px-4 text-white font-bold text-center text-3xl placeholder-gray-700 tracking-[0.5em]"
                        placeholder="------"
                    >
                </div>
                @error('otp')
            ...
            <div class="text-center mt-6">
                <a href="{{ route('password.verify.identity') }}" class="text-xs text-gray-500 hover:text-white font-semibold uppercase tracking-wider transition-colors">Resend Code / Change Identity</a>
            </div>
        </form>
    </div>
</div>
