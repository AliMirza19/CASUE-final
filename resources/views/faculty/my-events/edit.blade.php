...
@section('content')
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('faculty.my-events.index') }}" class="text-cause-purple hover:text-purple-700 font-medium flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-4xl mx-auto mb-8 border-l-4 border-yellow-400">
        <h4 class="text-yellow-800 font-bold mb-2">Revision Needed</h4>
        <p class="text-yellow-700 text-sm">Please update the event details below and submit again for approval. Be sure to check any feedback from the reviewers.</p>
    </div>

    <form action="{{ route('faculty.my-events.update', $event->id) }}" method="POST" id="eventForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-md p-6 flex justify-between items-center">
            ...
            <div class="flex space-x-4">
                <a href="{{ route('faculty.my-events.index') }}" class="bg-gray-200 text-gray-800 px-6 py-2 rounded-lg font-medium hover:bg-gray-300">
                    Cancel
                </a>
                <button type="submit" 
